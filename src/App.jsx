...
export default function App() {
  ...
  const getWeatherIcon = (condition) => {
    ...
    if (condition.includes("Partly") || condition.includes("Cloud")) return cloudy;
  ...
  const fetchWeather = async () => {
    ...
    try {
      const res = await fetch(
        `https://api.weatherapi.com/v1/forecast.json?key=50520c6face84d1bbbe123423252406&q=${trimmedCity}&days=3&aqi=no&alerts=no`
      );
        ...
        <div className="bg-white/20 shadow-lg p-8 rounded-lg mt-8 w-full max-w-[90%] md:max-w-[70%] lg:max-w-[50%] mx-auto">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {forecast.map((day, index) => (
              <div key={index} className="bg-blue-300 p-4 rounded-xl shadow-md w-36">
